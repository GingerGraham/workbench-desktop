#!/usr/bin/env bash
# tests/check-manifest-structure.sh — workbench-desktop
...
# list-*-releases are read-only interactive helpers -- must live in a
...
for fn in list-opendeck-releases list-noteshub-releases; do
    if grep -qE "^${fn}[[:space:]]*\(\)" "${REPO_ROOT}/shell/desktop.sh" 2>/dev/null; then
        ok "${fn} is in shell/desktop.sh (register.shell)"
    else
        fail "${fn} not found in shell/desktop.sh -- would be unreachable interactively if only in installers.sh"
    fi
done
