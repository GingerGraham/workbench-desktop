# Contributing to workbench-desktop

This is a `workbench` ecosystem module — it doesn't stand alone. It's
installed into a machine via `workbench-core`'s `wb add desktop`. Full
...
```sh
git clone https://github.com/GingerGraham/workbench-desktop.git
cd workbench-desktop
...
wb dev desktop
# — or directly:
wb track desktop --branch <your-branch>
```
...
Everything under `shell/`, `tests/` must run under Bash 3.2:
